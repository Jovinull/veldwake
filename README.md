...
Current status: **M3A multi-chunk correctness is merged; M3B streaming runtime is in planning only**. The Windows-first diagnostic client renders a deterministic static three-chunk fixture through `wgpu`/D3D12; it is an engineering proof, not yet a playable game or streaming world.
