...
Current status: **M3A multi-chunk correctness is merged; M3B1 headless streaming runtime is implemented on its feature branch, without presentation integration**. The Windows-first diagnostic client still renders the deterministic static M3A fixture through `wgpu`/D3D12; M3B1 is a CPU-only engineering proof, not yet a playable or camera-driven streaming world.
