...
| KI-010 | Open | M3B2 has automated headless coverage and log evidence from two release client runs, but no agent-performed visual inspection of seams, unloaded-as-AIR, re-entry, far teleport, or window lifecycle. | Owner performs the interactive smoke in `HANDOFF.md` before the M3B pull request is merged. |
