...
| KI-008 | Open | The audited Windows host has intermittently produced linker `LNK1104` for test executables when `cargo nextest` follows another Cargo gate, even sequentially. Recurred twice on 2026-09-16 during M3B work and twice consecutively during M3C0 (no holder process, files absent); the third plain run and a `--build-jobs 1` run both passed 86/86. During M3C1 it failed four consecutive times (including `--build-jobs 1`) before passing; a bounded retry loop of five plain runs is now the working procedure. | Verify no project/Cargo process is holding the named executable, then rerun `cargo nextest` alone. Do not clean artifacts or change code unless the isolated retry also fails. |
| KI-009 | Accepted | When a neighbor chunk arrives or changes, the affected center mesh is invalidated and its GPU mesh stops drawing until the replacement is meshed and uploaded, so chunks visibly blink during streaming. | Deliberate M3B policy: never draw a knowingly stale mesh. Revisit only with a design that can prove the old mesh exposes no incorrect seam. |
