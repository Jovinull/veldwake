...
| KI-008 | Open | The audited Windows host has intermittently produced linker `LNK1104` for test executables when `cargo nextest` follows another Cargo gate, even sequentially. Recurred twice on 2026-09-16 during M3B work; each isolated retry passed. | Verify no project/Cargo process is holding the named executable, then rerun `cargo nextest` alone. Do not clean artifacts or change code unless the isolated retry also fails. |
