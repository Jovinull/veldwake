...
`VELDWAKE_POSE` also accepts the four M8 landmark poses — `landmark-first-choice-a`, `landmark-first-choice-b`, `landmark-reveal` and `landmark-gate` — which are **derived from the landmark plan** rather than written down, so they photograph wherever the composition actually put the landmarks: the first two stand at the discovery overlook turned to each of the two landmarks visible from it, the third stands at the landmark that reveals the third one and turns to it, and the fourth stands ten columns out from the gate, square on to its opening. Each places the real follow camera, not a free camera.
...
| `initiative`, `initiative:<driver>`, `initiative:<driver>@<tick>` | combat initiative at the open clearing; `<driver>` is `read`, `spam` or `spam-read`, and `@<tick>` replays a driven fight headless to that tick and holds it (see [`../planning/COMBAT_INITIATIVE.md`](../planning/COMBAT_INITIATIVE.md)) |
| `weapon-choice`, `weapon-choice:<driver>[+found][@<tick>]` | the M9 weapon-choice laboratory: the same clearing and adversary as `initiative`, with the M9 exchange at a QA point beside the player's round start; `+found` has a driver take the found weapon through the real rule on its first armed tick |
...
While an encounter is armed: `WASD` moves relative to the camera, `J` or the left mouse button attacks, `K` or `Space` dodges, `E` interacts with the M9 exchange site, `F4` detaches the camera to look around, and `F1` cycles to a `combat` debug view that draws the hurt volumes and blade endpoints a hit is decided by. `VELDWAKE_POSE` also accepts the five combat poses — `combat-side`, `combat-close`, `combat-shoulder`, `combat-wide`, `combat-plan` — which, for a frozen moment, are placed against the two bodies rather than against the arena, so every moment is framed the same way wherever the fight drifted to. The client logs an `encounter ready` line at startup, an `audio device open` line when a device is found, and `combat state` and `combat work` lines every five seconds; all are described in [`../engineering/OBSERVABILITY.md`](../engineering/OBSERVABILITY.md).

`traverse` is the M7 session, and since M9 it also carries the weapon exchange. `E` interacts: standing within `1.75` world units of the weapon planted in the gate's opening swaps it for the one in the player's hand, and returning reverses the choice. A press out of range or while the body is busy is refused silently and counted in `interacts_refused` — **there is no prompt, icon, floating text or marker anywhere**, and the object in the world is the only thing that says it is there. The choice survives a defeat and its encounter reset (ARM-001) and does not survive a restart, because M9 is deliberately not persistence. When several combat verbs arrive in one authoritative tick the priority is **attack, then dodge, then interact**, and a press a higher verb consumed does not fire later. The client logs a `weapon exchange ready` line at startup with the site's column, its ground, the interaction radius and the measured signature beside the locked one, and `combat state` reports `player_weapon`, `site_weapon`, `interacts` and `interacts_refused`. See [`../planning/M9_MEANINGFUL_REWARD.md`](../planning/M9_MEANINGFUL_REWARD.md).

`weapon-choice` is test infrastructure for `OWNER PLAYTEST — WEAPON CHOICE MATTERS (REVISIT)`, not product content. It is the `initiative` fight exactly — same clearing, same starts, same adversary, same pressure lunge — plus the M9 exchange at a point `1.25` world units to the player's left at the round start. Every round, won or lost, resets both bodies to their starts, so the body always comes back beside the point, and `E` there swaps the carried weapon for the planted one; the choice survives the reset (ARM-001) and not a restart. Nothing marks the point but the planted weapon. It always stands at the clearing, whatever `VELDWAKE_INITIATIVE_SITE` says. The product exchange is still only in `traverse`, at the gate, and the adversary a player meets there still fights without combat initiative (KI-041, KI-043). See [`../planning/M9_MEANINGFUL_REWARD.md`](../planning/M9_MEANINGFUL_REWARD.md#m9-revisit).
...
Headless evidence comes from `cargo run --release -p veldwake-streaming --bin streaming-probe`, which accepts profile or source names to run a subset, from `cargo run --release -p veldwake-procedural --bin terrain-probe`, whose commands are `map`, `column`, `chunk`, `signature`, `bench`, `poses`, `vegetation` and `landmarks` — the last of which re-derives the landmark plan, refuses to print if the second derivation disagrees with the world's own, and reports the derivation's cost in the current build profile, from `cargo run --release -p veldwake-character --bin character-probe`, which takes `body`, `parts`, `skeleton`, `collision`, `palette`, `poses`, `contact`, `signature` and `bench` with an optional `--fixture golden|sturdy|varied`, and from `cargo run --release -p veldwake-combat --bin combat-probe`, which takes `weapon`, `spec`, `reach`, `moments`, `script`, `partition`, `signature`, `bench`, `trace`, `dodge`, `bodies`, `aim` and `contact [player|adversary] [golden|sandbox]` — `signature` prints all seven combat locks side by side with their measured values — the three M6 ones, `COMBAT_INITIATIVE_SIGNATURE` and the three M9 ones. `REWARD_BEHAVIOR_SIGNATURE` depends on the golden world and is asserted by its own client test rather than printed by the headless probe. None of the four needs a GPU, a window or an audio device.
