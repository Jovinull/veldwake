...
The client logs two aggregate `M3B streaming` lines every five seconds; there is no per-chunk logging. `F1` cycles the debug views (`off`, `lod`, `residency`, `boundaries`) and `F2` toggles their wireframe boxes; both are reported in the interval line as `debug_mode`, `debug_boxes`, `debug_draws`, and `debug_slots`, and the views are off by default so measurements stay comparable. `VELDWAKE_PROFILE` selects the streaming profile: `default` (M3B radius 1, `Lod0` only), `m3c-baseline` (radius 3, `Lod0` only), or `m3c-banded` (radius 3 with the `Lod0`/`Lod1` band); unknown values fall back to `default` with a warning. The headless streaming evidence comes from `cargo run --release -p veldwake-streaming --bin streaming-probe`.
