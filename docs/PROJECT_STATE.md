...
**M5 — Procedural Character is implemented on `feat/m5-procedural-character` and awaits independent branch QA.** Nothing is merged and no pull request is open. The branch adds one crate, no third-party dependency, one ADR, and a versioned character style contract; the milestone document records five design errors that captures rejected and the frames that rejected them. See [`planning/M5_PROCEDURAL_CHARACTER.md`](planning/M5_PROCEDURAL_CHARACTER.md) and [`agents/HANDOFF.md`](agents/HANDOFF.md).
