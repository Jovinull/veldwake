...
- M3D: an experimental disk cache of source results inside `crates/streaming`, on the worker thread. Explicitly a cache and not a save: every entry is reproducible from the source, nothing is authoritative, and no migration is promised. A 48-byte little-endian header carries magic, format version, chunk edge, cell encoding, entry kind, payload encoding, coordinate, source fingerprint, payload length, and an FNV-1a checksum; `CacheFormatError` has fourteen typed variants split into stale and corrupt. `DiagnosticChunkSource::fingerprint()` hashes an explicit rule descriptor plus a hand-maintained revision and is locked by a test. Keys are write-once per format version, fingerprint, and coordinate; publication is temp-and-rename without `fsync` because a checksummed discardable entry regenerates; rejected entries are deleted and republished; temporary residue is swept and counted on open; `KnownAbsent` is a typed entry and a missing file is always a miss. `RuntimeMetrics::cache` is separate from the streaming counters and is all zero without a cache. `VELDWAKE_CACHE_DIR` opts the client in; unset, nothing touches the filesystem. Measured on the audited host: run-length payloads are 169 times smaller than raw on this fixture (24,438 against 4,132,656 bytes for 81 entries) and faster to encode and decode, but raw stays the default because the fixture is 97% air and run-length has a 3× worst case. Warm is slower than no cache at all (13–37 ms against 6–7 ms) because the diagnostic source is trivial; the experiment proves the boundary and the cost, not a speedup (KI-015, KI-016).
...
No product world generation, player saves, authoritative world persistence, greedy meshing, generalized batching/instancing, origin rebasing, multiple workers, authoritative simulation, gameplay, physics, audio, networking, mod runtime, UI framework, or internal editor exists. LOD exists only as the M3C experiment on this feature branch, behind `VELDWAKE_PROFILE=m3c-banded` and never enabled by default; there is no production LOD policy. The finite diagnostic source is not a world generator, and the M3A static fixture is no longer rendered by the client (its voxel-crate tests remain).
