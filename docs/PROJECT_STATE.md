...
**M3B — Streaming Runtime is complete and merged. M3C0–M3C2 are implemented on `feat/m3c-lod-debug`: edge-generic grid and mesher, headless LOD core, GPU presentation of both levels, and the measured baseline-versus-LOD comparison. LOD is implemented but not the default (upload bytes +81%, CPU mesh time +139%, and a real GPU high-water-mark saving of only 19% once staged replacements are counted); LOD transitions no longer open holes (the committed mesh stays drawn until its transition group commits atomically on both sides; 909 presentation gaps → 0 on the same path); keyboard-toggled debug views (`F1`/`F2`) are implemented and off by default.** M3B merged through [PR #5](https://github.com/Jovinull/veldwake/pull/5) at merge commit `b5473dbb7836b65e6c6c5662a8abf6f02b6e8043`, and the Claude Code entry point through [PR #4](https://github.com/Jovinull/veldwake/pull/4) at `994e9863936441606d9bd675e1ea62bc74300bf9`; M3A merged through [PR #3](https://github.com/Jovinull/veldwake/pull/3) at `af1cabc913a9500eefbcd647a56c881d2c1288c0`. The repository remains a non-playable engineering proof.
...
- M3C2: `ChunkPresentation` carries the level and nothing else from streaming (now `stage_chunk`/`commit_staged`/`discard_staged`/`deactivate_chunk`/`remove_chunk`, see the follow-up); two-`vec4` `ModelUniform` (origin never scaled, `scale.x` 1 or 2) with `world = translation + local * scale`; per-level `GpuResidency`/`LevelResidency` and per-level upload totals; `StreamingConfig::m3c_baseline()`; `VELDWAKE_PROFILE` (`default`/`m3c-baseline`/`m3c-banded`) selects the client profile; benchmark reports include per-level counts, timings, render-submit timing, and `time_to_idle_ms`. Measured on the audited host: GPU bytes −57%, upload bytes +74%, CPU mesh time +41% → LOD not default.
- M3C2 follow-up (LOD transitions without holes): gap accounting by `InvalidationCause` (909 presentation gaps, mean ≈84 / max 232 frames, 62 simultaneous on the banded path; baseline 0); committed/target/pending separation on the record with `MeshStamp::geometry_key()` and `differs_only_by_presentation()`; `SeamContract` in `NeighborStamp` instead of raw presentation; presentation-only invalidations retain the committed mesh, data invalidations drop it; `transition_groups()` joined only across a drawn side whose level or seam contract changes (`seam_changes`); staged-then-committed presentation (`GpuChunkSlot` with `staged`/`staged_empty`, per-level `staged`/`staged_bytes`, `peak_staged_bytes`); coalescing by stamp; `ready_undrawn_*` and blocked-group counters that catch never-drawn entrants. Re-measured: 0 gaps, 0 ready-but-undrawn updates, 648 swaps, 548 uploads / 76,214,896 bytes (baseline 167 / 41,275,744), peak GPU bytes 4,954,960 (baseline 12,101,888), largest group 78, exit 0, no GPU errors. LOD still opt-in.
- M3C2 hardening: `ChunkPresentation::commit_staged_group` replaces `commit_staged` and is all-or-nothing (verify every member, then swap, or change nothing and return `PresentationCommitError`); `StreamingRuntime::commit_group` refuses a group that is not fully ready; the bridge asks the presentation first and marks nothing committed anywhere on refusal (`presentation_commit_failures`, `commit_invariant_failures`, both `0` on the benchmark path). GPU bytes are sampled from the presentation every update as `GpuBytes { committed, staged }` with three peaks, the total taken on the simultaneous sum: baseline 12,101,888 versus banded 9,800,128 (−19%), correcting the −57%/−59% committed-only figures.
- M3C3: `debug` module and one `LineList` pipeline. `F1` cycles `Off`/`Lod`/`Residency`/`Boundaries`, `F2` toggles boxes. `Lod` tints `Lod1` through the camera uniform; `Residency` draws one box per tracked coordinate colored by record state and inset by demand ring, from the new `StreamingRuntime::tracked_states()`; `Boundaries` draws presented chunk boxes by level, mixed-level seam faces from the fine side, pending/staged replacements, and uncommitted transition groups with large ones highlighted (KI-011). Off costs nothing: 0 draws, 0 uniform writes, 60.0 FPS, mesh upload budget untouched. 637 residency boxes cost 15–38 FPS on the audited host (KI-012).
...
**M3C — Initial LOD + Streaming Debug Visualization:** M3C0–M3C3 are implemented and measured; LOD stays behind `VELDWAKE_PROFILE=m3c-banded`, no longer opens holes, and its GPU saving is now known to be 19% rather than 57% (KI-013). Next: the swap-churn investigation (wider band, per-crossing coalescing, level-aware budgets, group size bound with proof — KI-011) re-measured on the same benchmark path before any default change. See the M3C section of [`planning/M3_STREAMING_WORLD.md`](planning/M3_STREAMING_WORLD.md). World generation, saves/cache, ECS, gameplay, physics, networking, multiple workers, origin rebasing, render graph, generalized batching, and biomes remain excluded.
