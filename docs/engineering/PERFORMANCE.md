...
The M3C2 follow-up (transition model) re-measured the same path on the same day: presentation gaps 909 → 0 and ready-but-undrawn updates 0 over 7,805 updates for `m3c-banded`, with 648 level swaps, 1,388 mesh jobs, 548 uploads / 76,214,896 bytes (baseline 167 / 41,275,744), peak GPU bytes 4,954,960 (baseline 12,101,888, −59%), peak staged (undrawn) GPU bytes 4,650,960, largest transition group 78 chunks, CPU mesh time on the path 380,377 µs against a same-day baseline of 164,698 µs (baseline totals varied 164,698–274,069 µs across runs), time to idle 13.3 s for both, render-submit max 17.8 ms, no GPU validation error. The upload-bytes rule still fails (+85%), so LOD remains opt-in; the model buys correctness of transitions, not throughput.
