...
The workspace has **402 tests**: 38 voxel, 69 procedural, 97 streaming, 111 character, 87 client. Every pre-M5 test is unchanged in meaning.
...
The client's 87 tests keep the boundary honest without a GPU: the identifier ranges of all three content domains are proved disjoint where all three tables are visible, the ground adapter is proved to return block tops and to keep absence absent, the portrait clearing is proved level, open and free of vegetation, the terrace stand is proved to have a step in front of the toes, every camera pose is proved to stand above ground at both stand points, and each of the three diagnostic courses is proved to stay in the region, stay dry, close into a loop, and — for the two that are meant to — either stay level or actually climb.
