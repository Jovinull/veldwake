...
**Did the historical encounter move?** Nothing new may reach it. `every_historical_attack_recovers_the_same_whatever_it_meets` pins the outcome-dependent recovery to a no-op for every historical spec; `without_a_lunge_its_stagger_is_answered_the_historical_way` pins the brain; and the untouched locks — `GOLDEN_ENCOUNTER_SIGNATURE`, both weapon fingerprints, every M5/M7/M8 signature — are the rest of the answer. The new behaviour is locked separately by `the_combat_initiative_reference_is_locked`.

**Does the owner's strategy still win?** `crates/combat/src/oracle.rs` plays three policies — owner-spam, spam-read and read at lags `18`, `24`, `32` and `48` — through the real rules under six seeds. The **control** comes first: `owner_spam_beats_the_historical_encounter_every_time_untouched` requires the historical `6/0` at exact per-seed tick counts, so the oracle is known to measure what it measured before the capability existed. `with_initiative_reading_beats_spam_and_spam_loses_more_than_it_wins` is the gate: owner-spam wins at most two fights in six and fewer than it loses while its opponent lands both attacks, spam-read wins more than owner-spam, and at every lag up to `32` read wins at least five, punishes more than spam-read, wastes fewer swings and ends healthier than owner-spam; across all of them no fight is a stalemate, no body moves more than `0.45` u in a tick, no stretch without a hit reaches `600` ticks, and no adversary dodge ever starts inside an unresolved player swing.

**Is the lunge honest?** Every claim about it is a probe of one lunge through the real rules, stepped across the band at `0.02` or `0.05`: a body that does nothing is hit everywhere in the band and missed just past it (no bluff); a player charging in with any of eleven misjudgements is hit first inside the band; swinging on the spot is hit; stepping aside — standing or walking in, walking or dodging, from both sides — escapes at the lags the fairness table claims; a missed lunge leaves an opening a `400` ms reader still takes; a connected one lets the adversary space before the player can act. `the_lunge_blade_is_level_and_on_the_line_while_it_can_connect` asserts the pose the no-bluff result rests on.

**Is the spacing dodge non-reactive?** Structurally, not statistically: `the_lunge_decision_does_not_depend_on_what_the_player_is_doing` and `its_own_stagger_is_answered_with_a_spacing_dodge_whatever_the_player_does` hold the brain's own state fixed and vary only the player's action — free, winding up, live, recovering, dodging — and require the same decision every time. `CombatCounters::dodges_during_unresolved_swing` is the runtime audit, asserted `0` in every oracle fight (COMBAT-005).
