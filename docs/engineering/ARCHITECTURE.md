...
- `crates/streaming`: headless CPU residency and detached-meshing orchestration. It depends only on `veldwake-voxel` and `std`, owns the finite diagnostic source and authoritative resident diagnostic chunks, validates its configuration, bounds eviction finalization, and exposes a per-frame `ResidencySummary`. It contains no camera, platform, renderer, GPU, or gameplay types. Since M3D it also owns an optional, experimental disk cache of source results (`cache::{format, store}`), confined to the worker thread inside the load path. That cache is discardable by definition and is not authoritative persistence: a later world-editing milestone owns that problem separately. No crate was split out for it, judged against the crate test above: one consumer, no build isolation need, no dependency inversion. `veldwake-voxel` remains filesystem-free, and the cache format reads cells through its public API rather than duplicating the voxel representation.
