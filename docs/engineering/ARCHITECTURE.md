...
### The armament boundary (M9, ported by the M9 revisit)

M9 added the first state whose lifetime is longer than an encounter and shorter
than a process: which of two weapons the player is carrying. Three seams, each
one deliberate, and [ADR-0011](../adr/0011-session-acquired-state-in-the-authoritative-encounter.md)
records the whole argument.

**The armament is authority, not presentation.** The weapon in the player's hand
selects the blade the sweep follows, the attack spec that times the action and
the aim-assist range, so it lives in `Encounter`. `ArmamentState::exchange` is
`pub(crate)` and the exchange rule is its only caller; the client reads an
armament to decide which uploaded mesh gets which matrix, and has no way to
write one.

**The world contributes a position and nothing else.** `WorldContact` gained
`weapon_exchange_site: Option<Vec2>` and one constructor that supplies it. The
world says *where*; the domain owns the radius, the refusals and the event. It
is deliberately not a trait, not an object, not an identifier and not a query —
a generic interaction seam is how an entity model arrives without anyone
deciding to build one.

**The anchor is the client's, derived from geometry the world already has.**
`apps/client/src/reward.rs` reads the gate's `crown_column` — which M8 recorded
is the lintel over its opening — steps one column along the gate's span axis,
and asks the client's own ground query for the height. `veldwake-procedural` is
unchanged by M9 and knows nothing about weapons, exchanges or players, exactly
as M8's landmark boundaries require. No chunk byte changes, so no chunk-source
fingerprint changes and no cache key is invalidated; the M9 identity is its own
`REWARD_BEHAVIOR_SIGNATURE` in the client.

**Where a swing reaches is production code, not a fixture.** `crates/combat/src/reach.rs`
holds `attack_envelope`, moved out of `fixture.rs` because the encounter's
aim-assist range is now derived from the weapon actually held. The fixtures, the
probe and the domain call one function; a fixture may not be a dependency of a
rule, and two copies of one formula is how the copy nobody tests drifts.

**Armament and attack kind are orthogonal** (the M9 revisit). Combat
initiative made `Encounter::attack_spec_for(side, kind)` the one place a kind
becomes a spec; the revisit put the armament in exactly one place under it,
`primary_spec(Player)`. The player only ever swings `Primary`, with whichever
weapon it holds; the adversary's `Primary` is always historical and its
`Pressure` is always the `PressureSpec`, and neither passes through the
armament. The found weapon is armament, not a third kind. Aim assist follows
the same split: a primary is aimed within its weapon's range
(`Encounter::aim_range_of`, `max(2.90, connects out to)`), a lunge within the
band's far edge, whatever the player holds.

**The weapon-choice laboratory is client configuration, not a second site
type.** `VELDWAKE_ENCOUNTER=weapon-choice` is the combat initiative setup plus
the reward, and its exchange point is a position the client resolves beside the
round start (`encounter::resolve_exchange`); the domain sees the same
`WorldContact::terrain_with_weapon_exchange` it sees for the gate.
