...
## Continue here — the M9 revisit failed its owner gate; the retune is the owner's decision

**`OWNER PLAYTEST — WEAPON CHOICE MATTERS (REVISIT)`: FAIL, 2026-09-23.** The owner's words are verbatim in [`../planning/M9_MEANINGFUL_REWARD.md`](../planning/M9_MEANINGFUL_REWARD.md#result--2026-09-23). The found weapon was perceived as better overall and easier to hit with; the original offered no practical advantage the owner could name and felt *more* dangerous; running in and attacking still did not work with either weapon, so combat initiative is not in question.

**The next step is the owner's, not an agent's.** The causal investigation recorded in the milestone document (*Causal investigation — why the found weapon's cost is not felt*) quantifies why the found weapon's commitment is not paid in play and compares found-weapon-only levers with the locks each would move. Do not implement any of them, do not change the found weapon's windup, active, recovery, damage, reach, step-in, knockback or geometry, and do not touch the adversary, the lunge or `COMBAT_INITIATIVE_SIGNATURE`, until the owner chooses. No pull request, no merge, no independent QA.
