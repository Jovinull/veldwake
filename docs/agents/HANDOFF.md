...
**Combat Pressure is BLOCKED on `feat/combat-pressure`, and the next step is an owner decision, not code.** It started from `main` at `ee35f62f97afbe3d001a27a576e9bae21e77c4d2`, which is M8 merged through PR #12; the M8 paragraphs below predate that merge. The owner accepted the capability — lateral evasion through the existing `Action::Dodge` plus a punish legal only against an observable player whiff — and declined to accept reaction `18`, punish damage `48` or a mandatory shared dodge as its tuning. Hard stop 1 fired on the first measurement: reacting to the player's swing escapes it at `18` ticks and at no separation from `24` ticks up, for the player's dodge and for both shorter sidesteps the owner named, because the player's windup is `22` ticks. Read [`../planning/COMBAT_PRESSURE.md`](../planning/COMBAT_PRESSURE.md) first: it has the numbers, the nine hard stops, what was deliberately not built, and four options for the owner, none chosen. `crates/combat/src/oracle.rs` is the only code; `owner_spam_beats_the_historical_encounter_every_time_untouched` is the M9 owner finding as a repository test and is expected to fail the day the encounter is deliberately changed. Do not build an adversary dodge, a punish attack or a response configuration until the owner chooses a way past the cliff, and do not tune damage to get around it. `feat/m9-meaningful-reward` is untouched and still waits on this capability. KI-037 to KI-039 belong to that branch, which is why this one records KI-040.
