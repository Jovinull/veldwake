...
- **Frozen replay for a moment shorter than a reaction.** On `feat/combat-initiative-spacing`, `VELDWAKE_ENCOUNTER=initiative:<driver>@<tick>` (`read`, `spam` or `spam-read`) replays the oracle fight headless from arming to that tick against the same terrain adapters the live session uses, then holds it; frame it with `VELDWAKE_POSE=combat-side` or `combat-plan`. A lunge's active window is `14` ticks, about a tenth of a second, and no burst triggered from a log line lands inside it reliably; a replay lands on the tick every time. Two captures three seconds apart must still be identical. Do not use `combat-shoulder` as the player's view: it frames from behind the adversary (KI-042).
- **Log-triggered bursts for live play.** In an initiative session `RUST_LOG=info,combat_event=debug` emits one line per `CombatEvent` — tick, event, side, attack kind, distance, both positions, the adversary's facing and both healths — from the `combat_event` target, which the default filter leaves off. Tail it for the event under test (a lunge start, a whiff, an adversary dodge) and fire the capture or the key from there. Budget `250`–`400` ms between the line and the key on this host; that is the reaction a harness has, and it is the one the fairness numbers had to cover.
- **Site selection for witnesses.** `VELDWAKE_INITIATIVE_SITE=spire-west` stands the initiative session at the M8 adversary's column with the spire behind the adversary, for the KI-041 witness. The default, `clearing`, is the owner's.
