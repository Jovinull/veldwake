...
| a second, silent notion of "the region" | The extent is interpreted twice — once as a continuous half-open rectangle for the veto, once as integer column indices for the grid — and they agreed by construction with nothing saying so. `the_grid_indexes_exactly_the_region_its_bounds_describe` now says it, at both corners and in both directions. |
...
The workspace has **699 tests**, three of them `#[ignore]`d and run deliberately — all three were run and all three pass: the M6 listening fixture, the whole-region reachability report, and the adversary placement re-derivation. Branch QA added nine: three ground-and-water oracles against generated voxels, two route properties, an independent placement oracle, one agreement test between the grid's indices and the region's bounds, and two adversarial `MoveBlockReason` cases.
