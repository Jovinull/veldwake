...
**M3C — Initial LOD + Streaming Debug Visualization is implemented on its feature branch (M3C0 grid/mesher, M3C1 headless LOD core, M3C2 GPU presentation plus measurement, the atomic transition model and its commit/GPU-accounting hardenings, and M3C3 keyboard-toggled debug views; LOD stays opt-in because the recorded rule fails on upload bytes and CPU mesh time and its real GPU saving is 19%, not 57%):** an edge-generic dense grid and mesher (M3C0), one coarse 2× power-of-two level with a coarse-occupancy seam rule, distance selection with chunk-unit hysteresis, level-aware mesh stamps, per-level metrics against a no-LOD baseline with a recorded decision rule, and keyboard-toggled wireframe/tint debug views without a UI framework. The cache/persistence experiment remains later M3 scope. See the M3C section of [`M3_STREAMING_WORLD.md`](M3_STREAMING_WORLD.md).
