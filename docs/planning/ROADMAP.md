...
**Implemented on `feat/m8-discoverable-landmarks`, accepted by the owner on 2026-09-22, and reviewed by independent branch QA.** Not merged, no pull request open. Every gate is green — 755 tests by default and 758 with the ignored fixtures, clippy, fmt, doc tests, rustdoc, metadata, deny, audit and eleven driven client runs on the audited host — and the composition, the numbers, the locks it moved, the one defect QA found and what nobody could verify are in [`M8_DISCOVERABLE_LANDMARKS.md`](M8_DISCOVERABLE_LANDMARKS.md).
