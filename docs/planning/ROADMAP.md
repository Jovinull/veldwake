...
**Next milestone. Nothing of it exists yet.** No character code, descriptor, skeleton, animation, collision representation, or dependency has been added; the scope above is the accepted statement and has deliberately not been turned into an architecture. Documentation-only preparation lives on `feat/m5-procedural-character`; [`../agents/HANDOFF.md`](../agents/HANDOFF.md) carries the cold-start reading order and the list of decisions that are still open.
