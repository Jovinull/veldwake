...
**Implemented on `feat/m8-discoverable-landmarks`, accepted by the owner on 2026-09-22, and waiting for independent branch QA.** Not merged, no pull request open. Every gate is green — 746 tests by default and 749 with the ignored fixtures, clippy, fmt, doc tests, deny, audit and a driven capture smoke on the audited host — and the composition, the numbers, the locks it moved and what this agent could not verify are in [`M8_DISCOVERABLE_LANDMARKS.md`](M8_DISCOVERABLE_LANDMARKS.md).
