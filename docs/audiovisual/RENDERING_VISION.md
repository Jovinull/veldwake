...
7. Something worth looking *at*. **Implemented in M8, on its branch:** generated landmarks drawn through the ordinary chunk path — no new pass, no new pipeline and no new uniform — with their own material table in their own `VoxelId` range and one ordered lookup in the renderer, so the style bible and [`LANDMARK_STYLE.md`](LANDMARK_STYLE.md) remain the only source of a colour. The interesting rendering question it answered is not how to draw one but whether one **reads**: a silhouette is composed against the sky, at a distance the fog leaves legible, at an elevation the default camera frames, and the client owns a presentation oracle that measures exactly that against the real camera, projection and fog ([ADR-0009](../adr/0009-two-level-landmark-visibility-and-eager-world-plan.md)).
8. Later evidence-driven culling/batching/GPU-driven techniques.
...
M8 exercised the second sentence from the other end. A landmark is built from terrain-scale voxels like the ground it stands on, and what distinguishes it is silhouette and value rather than resolution: a slenderness a hill cannot have, a crown against the sky, and a palette darker than lit grass and lighter than shadowed rock. The measurement that matters is not how many voxels it has but how many of them a viewer can see from where the composition intends — `21.3` and `22.0` voxels of silhouette for the two landmarks a player first chooses between, against a minimum of `8`.
